...
| `name` | string | ◯ | 商品名 |
| `code` | string | - | 商品コード。インフォマート連携時に取得、書き起こし/棚卸登録商品は空文字列の場合あり |
| `route` | enum | ◯ | 登録元の内部分類（後述。J-004 で UI 表示は撤廃、データモデルとしては保持） |
| `supplier` | string | ◯ | 発注先名。棚卸登録商品は `(棚卸登録商品)` または `-` |
...
| `in-house`(棚卸登録商品) | `false` | `true` | 顧客が棚卸商品として作成 |
...
> ※ J-004 で UI 表示は撤廃。本 enum はデータモデル・内部ロジック判定（例: 編集画面の費目欄表示可否＝`route='in-house'` のみ）用として保持。表示色（route-badge）も削除済み。

| 値 | 内部ラベル | 説明 |
|---|---|---|
| `informart` | インフォマート連携 | インフォマート受発注からのマスタ自動同期 |
| `delivery` | 納品書書き起こし | 納品書(FAX含む)からの書き起こし登録 |
| `order` | 発注アプリ連携 | アスクル等の発注アプリ連携によるマスタ取込 |
| `in-house` | 棚卸登録商品 | 新規追加画面から手動で登録した棚卸登録商品 |
...
- **保存先:** `shop_item_expenditure_types.expenditure_type`（商品×費目の割り当て）。`is_ai_generated` でAI付与/手動を区別。
  ...
  - 棚卸登録商品 → 新規追加画面で手動選択（`is_ai_generated=false`）
