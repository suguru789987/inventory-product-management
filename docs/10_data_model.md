...
## category（費目）— 仕入商品の固定3区分

棚卸商品の費目。**棚卸対象は仕入れた商品に限られる**ため、品目区分は固定enumとし、選択肢は **食材 / 飲料 / 資材** の3つのみ。
店舗ごとの変動費目（`monthly_cost_items`）は引かない。
...
- **モック:** `categoryOptions = ['食材', '飲料', '資材']`。

> ⚠️ **要確認（OQ-11）:** `expenditure_type`(int) のenum定義（コード→ラベル対応）を本番アプリコードから提示してもらう。固定3区分（食材/飲料/資材）にどう対応付けるか確定が必要。
